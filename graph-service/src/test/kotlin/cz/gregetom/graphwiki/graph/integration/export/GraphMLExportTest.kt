...
class GraphMLExportTest : AbstractIntegrationTest() {
    ...
    fun graphMLExportTest() {
        ...
        val response = httpGet.doGetAndReturnResponse(ExportApiLinks.graphML(entryPoint.id).toUri())

        assertThat(response.contentLength).isGreaterThan(0)
        assertThat(response.contentType).isEqualTo(MediaType.APPLICATION_XML_VALUE)
    ...
    fun graphMLExportInactiveVertexTest() {
        val entryPoint = personDataSupport.randomInactivePerson()
        val person2 = personDataSupport.randomActivePerson()
        relationshipDataSupport.randomActiveRelationship(entryPoint.id, person2.id)

        httpGet.doGetAndExpect(ExportApiLinks.graphML(entryPoint.id).toUri(), HttpStatus.NOT_FOUND, TestUsers.USER)
        httpGet.doGetAndExpect(ExportApiLinks.graphML(entryPoint.id).toUri(), HttpStatus.OK, TestUsers.ADMIN)
    }
}
